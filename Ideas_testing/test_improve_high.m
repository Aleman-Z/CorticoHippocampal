...
%function [h]=plot_inter_conditions_33(Rat,nFF,level,ro,w,labelconditions,label1,label2,iii,P1,P2,p,timecell,sig1_nl,sig2_nl,ripple_nl,carajo_nl,veamos_nl,CHTM2,q,timeasleep2,RipFreq3,RipFreq2,timeasleep,ripple,CHTM,acer,block_time,NFF,mergebaseline,FiveHun,meth,rat26session3,rat27session3,notch)
...
%   PPP= [PPP {AV(:,:,z)} ];
% end
[PPP]=lowpassrip(p,w,ro);
[PPP_nl]=lowpassrip(p_nl,w,ro);

% AV=mat2cell(PP,[3 2401]);
% for z=1:length(p)
%     
% end
...
freq1=justtesting(PPP_nl,create_timecell(ro,length(p_nl)),[1:0.5:30],w,10,toy);
freq2=justtesting(PPP,create_timecell(ro,length(p)),[1:0.5:30],w,0.5,toy);
% 
...
% %     cfg.colorbar       = 'yes';
%%
% av=squeeze(mean(freq1.powspctrm(:,w,:,:),1));
% I=imagesc(av); 
% colorbar(); 
% colormap(jet(256))
...
%%
%hIGH PASS
[QQQ]=lowpassrip(q,w,ro,1);
[QQQ_nl]=lowpassrip(q_nl,w,ro,1);
...
%%
if ro==1200
freq3=barplot2_ft(QQQ_nl,create_timecell(ro,length(q_nl)),[100:1:300],w,toy);
freq4=barplot2_ft(QQQ,create_timecell(ro,length(q)),[100:1:300],w,toy);
else
freq3=barplot2_ft(QQQ_nl,create_timecell(ro,length(q_nl)),[100:2:300],w,toy); %Memory reasons
freq4=barplot2_ft(QQQ,create_timecell(ro,length(q)),[100:2:300],w,toy);
end
%%

freq3=justtesting(p_nl,create_timecell(ro,length(p_nl)),[100:2:300],w,10,toy);
freq4=justtesting(p,create_timecell(ro,length(p)),[100:2:300],w,0.5,toy);
