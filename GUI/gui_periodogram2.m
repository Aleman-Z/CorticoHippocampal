...
if scoring==1 && ~isempty(A) && ~isempty(find(transitions(:,1)==3))
[pfc,~]=reduce_data(PFC,transitions,1000,3);
[hpc,~]=reduce_data(HPC,transitions,1000,3);

else
    pfc=cell(size(a1,1),1);
    hpc=cell(size(a1,1),1);
    for h=1:size(a1,1)
    pfc{h,1}=PFC(a1(h,1):a1(h,2));
    hpc{h,1}=HPC(a1(h,1):a1(h,2));
    end    
end
