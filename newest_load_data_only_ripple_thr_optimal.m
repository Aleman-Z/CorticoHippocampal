...
vq = interp1(y1/(timeasleep*60),DEMAIS,[0.28 0.80 2.8])
if sum(isnan(vq))>0
vq = interp1(y1/(timeasleep*60),DEMAIS,[0.28 0.80 max(y1/(timeasleep*60))])    
end
...
[S2{k}, E2{k}, M2{k}] = findRipplesLisa(signal, ti.', vq(k) , (vq(k))*(1/2), []);
