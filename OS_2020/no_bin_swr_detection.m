function [nr_swr_HPC, nr_swr_Cortex,nr_cohfos,nr_single_hpc,nr_single_cortex]=no_bin_swr_detection(HPC,Cortex,states,ss,D1,D2,xx,yy,fn)
i=1;
...
clear n
              ...
              [HPC_binned, Mx_hpc] =  epoch_ephys_states(HPC.',states,ss,a1,a2,b1,b2,D1,yy{1},fn);
              [Cortex_binned,Mx_cortex] = epoch_ephys_states(Cortex.',states,ss,a1,a2,b1,b2,D2,xx{1},fn);
               nr_swr_HPC=sum(cellfun('length',HPC_binned(:,1)));
               nr_swr_Cortex=sum(cellfun('length',Cortex_binned(:,1)));  
               
               
     %Coocurrent
                 [cohfos1,cohfos2]=cellfun(@(equis1,equis2) co_hfo(equis1,equis2),Mx_hpc,Mx_cortex,'UniformOutput',false);
                 %if ~isempty(cohfos1{n})
                      nr_cohfos(i)=sum(cellfun('length',cohfos1(:,1)));                          
                 %else
                 %    nr_cohfos(i,n)=0;                          
                 %end

                  v2_single_hpc=cellfun(@(equis1,equis2) single_hfo_get_sample(equis1,equis2),Mx_hpc,cohfos1,'UniformOutput',false);
%                  if ~isempty(v2_single_hpc)
                      nr_single_hpc(i)=sum(cellfun('length',v2_single_hpc(:,1)));                          
%                  else
%                      nr_single_hpc(i,n)=0;                          
%                  end

                  v2_single_cortex=cellfun(@(equis1,equis2) single_hfo_get_sample(equis1,equis2),Mx_cortex,cohfos2,'UniformOutput',false);
%                  if ~isempty(v2_single_cortex{n})
                      nr_single_cortex(i)=sum(cellfun('length',v2_single_cortex(:,1)));                          
%                  else
...
%                            [HPC_binned{n}, Mx_hpc{n}] =  epoch_ephys_states(HPC(:,1+(n-1)*bin_size2:bin_size2*(n)).',slpscr_binned{n},ss,a1,a2,b1,b2,D1,D2,yy{1},fn);
