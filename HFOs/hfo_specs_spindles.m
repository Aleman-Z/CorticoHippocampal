function [x,y,z,w,h,q,l,p]=hfo_specs_spindles(si,timeasleep,fn,print_hist)
    ...
    if ~isempty(si)
        ...
        if print_hist==1
            subplot(3,2,1)
%             xlim([100 250])
            histogram(x,10); title('Instantaneous Frequencies');xlabel('Frequency (Hz)');ylabel('Count')
%             ylim([0 35])
%             yticks([0:5:35])
...
%         si_mixed.i2=find(y>th);
        ...
        if print_hist==1
            subplot(3,2,2)
%             xlim([100 250])
            histogram(y,10); title('Average Frequencies');xlabel('Frequency (Hz)');ylabel('Count') 
%             ylim([0 50])
...
%             xticks([0 0.025 0.05 0.075 0.1]*1000)
            histogram(q*1000,10); title('Duration');xlabel('MIliseconds');ylabel('Count')    
%             ylim([0 40])
%             yticks([0:10:40])
