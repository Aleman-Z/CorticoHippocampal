...
%%
xo
%% Histograms of interriple lenghts. 
F=LL_clean(Aver);
Ca = [F{1} F{2}  F{3}  F{4}];
grp = [zeros(1,length(F{1})),ones(1,length(F{2})),2*ones(1,length(F{3})),3*ones(1,length(F{4}))];

bb=boxplot(Ca,grp,'Notch','on' );
ylabel('Time (sec)')
ave=gca;
...
string=strcat('InterrippleTime_','Allconditions','_',Block{block_time+1},'_','.pdf');
figure_function(gcf,[],string,[]);
string=strcat('InterrippleTime_','Allconditions','_',Block{block_time+1},'_','.eps');
print(string,'-depsc')
string=strcat('InterrippleTime_','Allconditions','_',Block{block_time+1},'_','.fig');
saveas(gcf,string)
%% Violin 
 violin(F.',[],'facecolor',[[105/256 105/256 105/256];[38/256 43/256 226/256];[1 1 0];[0 0 0]],'medc','k','mc','')
legend off
ave=gca;
...
string=strcat('InterrippleTime_Violin_','Allconditions','_',Block{block_time+1},'_','.pdf');
figure_function(gcf,[],string,[]);
string=strcat('InterrippleTime_Violin_','Allconditions','_',Block{block_time+1},'_','.eps');
print(string,'-depsc')
string=strcat('InterrippleTime_Violin_','Allconditions','_',Block{block_time+1},'_','.fig');
saveas(gcf,string)
%%
ksdensity(F{1})
hold on
