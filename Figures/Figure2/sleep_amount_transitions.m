...
L=sum(vee(:,2)-vee(:,1))/60; %6 hours?
 L_sleep=sum(vee(find(transitions(:,1)~=1),2)-vee(find(transitions(:,1)~=1),1))/60; %Wake

%find(transitions(:,1)==3);

 g1=sum(vee(find(transitions(:,1)==1),2)-vee(find(transitions(:,1)==1),1))/60; %Wake
 g3=sum(vee(find(transitions(:,1)==3),2)-vee(find(transitions(:,1)==3),1))/60; %NREM
 g4=sum(vee(find(transitions(:,1)==4),2)-vee(find(transitions(:,1)==4),1))/60; %Transitional Sleep
 g5=sum(vee(find(transitions(:,1)==5),2)-vee(find(transitions(:,1)==5),1))/60; %REM
