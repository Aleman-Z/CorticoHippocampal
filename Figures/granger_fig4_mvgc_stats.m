...
hold on
plot(mean(gr2,2))
vp=max(pp.CurrentAxes.YLim);

rrc=area(not(F1)*vp,'FaceColor','none')%Right RF-BF Cocontraction
set(rrc, 'FaceColor', 'r')
alpha(0.1)
xlim([0 300])
%%
...
rrc=area(O2,'FaceColor','none')%Right RF-BF Cocontraction
%%
% %
boxplot(gr1.')
figure()
...
%  [o1(x),o2(x)]=ttest2(gr(1,:),gr(x,:));  
 [o1(x),o2(x)]=ttest2(gr1(x,:),gr2(x,:));
 [O1(x),O2(x)]=ttest2(ggr1(x,:),ggr2(x,:));
 [A1(x),A2(x)]=signrank(gr1(x,1:373),gr2(x,1:373));
 [A3(x)]=signrank(gr1(x,1:373)-gr2(x,1:373));
 [O3(x),O4(x)]=ttest(ggr2(x,1:373)-ggr1(x,1:373));
 
 
[F1(x),F2(x)] = vartest2(ggr1(x,1:373),ggr2(x,1:373));

coj(x,:)=ggr2(x,1:373)-ggr1(x,1:373);
Coj(x,:)=gr2(x,1:373)-gr1(x,1:373);
